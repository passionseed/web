...
export const MapViewerNode = ({
  ...
  if (!isUnlocked) {
    ...
    if (isCompleted) {
      ...
      // Submitted awaiting grade (if not marked as complete by isCompleted)
...
export const ViewerCommentNode = ({
  data,
  selected,
}: {
  data: MapNode & {
  ...
  const { userRole, isInstructorOrTA, onDataChange } = data;
