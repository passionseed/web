...
export interface MapNodeDefaultData extends MapNode {
...
const MapNodeDefault = ({
  data,
  selected,
}: NodeProps & { data: MapNodeDefaultData }) => {
  const {
    progress,
    isUnlocked = false,
    isCompleted = false,
  ...
  const spriteUrl = sprite_url || "/islands/crystal.png";
  ...
  if (!isUnlocked) {
    ...
    // Check passed isCompleted which is calculated in parent
    if (isCompleted) {
    ...
    <div className={`relative group ${animationClass}`}>
      ...
      <div className="relative min-w-[64px] min-h-[64px] w-fit h-fit flex items-center justify-center">
        ...
        <span className="sr-only">
          {title} node, difficulty {difficulty}, status:{" "}
...
export default memo(MapNodeDefault);
