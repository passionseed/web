...
export interface MapNodeData extends MapNode {
  ...
  // Add other properties that might be passed
  [key: string]: any;
}

interface DefaultMapNodeProps {
  data: MapNodeData;
  selected?: boolean;
}

export const DefaultMapNode = memo(({ data, selected }: DefaultMapNodeProps) => {
  const {
    progress,
    isUnlocked = false,
    isCompleted = false,
    ...
    <div className={`relative group ${animationClass}`}>
      ...
      <div className="relative min-w-[64px] min-h-[64px] w-fit h-fit flex items-center justify-center">
        ...
        <div
          ...
          } transition-all duration-300 group/label`}
        >
...
DefaultMapNode.displayName = "DefaultMapNode";
