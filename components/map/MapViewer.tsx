...
import { nodeTypes } from "./MapViewerNodeTypes";
...
export function MapViewer({ map, seedRoomId, seedTitle, seedId, roomSettingsComponent }: MapViewerProps) {
  ...
  const isNodeUnlocked = useCallback((nodeId: string): boolean => {
    ...
    // Use pre-fetched incoming paths (O(1) lookup per node)
    const incomingPaths = nodeData.node_paths_destination;

    // Fail-Closed: if no paths array is present, assume locked (unless we trust it to be [] if empty)
    if (!incomingPaths) return false;

    // If no prerequisites, node is unlocked
    if (incomingPaths.length === 0) return true;

    // Check if ALL prerequisites are passed OR submitted
    return incomingPaths.every((path: any) => {
      const progress = progressMap[path.source_node_id];
      return progress?.status === "passed" || progress?.status === "submitted";
    });
  }, [map.map_nodes, isInstructorOrTA, progressMap]);
  ...
  }, [reactFlowInstance, selectedNode, isNodeUnlocked]);
  ...
  useEffect(() => {
    const transformedNodes = map.map_nodes.map((node) => {
      ...
      // Optimization: Calculate unlocked status using node_paths_destination directly
      // This avoids O(N) search for each node, making transformation O(N) instead of O(N^2)
      ...
      if ((node as any)?.node_type === "text") {
         ...
         const incomingPaths = node.node_paths_destination;

         // Fail-Closed: if no paths array is present, assume locked
         if (!incomingPaths) {
            isUnlocked = false;
         } else if (incomingPaths.length === 0) {
            isUnlocked = true;
         } else {
             isUnlocked = incomingPaths.every((path: any) => {
                 const progress = progressMap[path.source_node_id];
                 return progress?.status === "passed" || progress?.status === "submitted";
             });
         }
      ...
      const submissionRequirement = (node.metadata as any)?.submission_requirement || "single";

      if (submissionRequirement === "single") {
        isCompleted = progress?.status === "passed" || progress?.status === "submitted";
      } else {
        // All requirement
        if (progress?.member_progress) {
          isCompleted = progress.member_progress.every(
            (member: any) => member.status === "passed" || member.status === "submitted"
          );
        } else {
          isCompleted = progress?.status === "passed" || progress?.status === "submitted";
        }
  ...
  }, [map, progressMap, setNodes, setEdges, isInstructorOrTA, isTeamMap, userRole, selectedNode]);
