  ...
  const renderedContent = useMemo(() => {
    ...
    const flushTextBlock = (endLine: number) => {
      if (currentTextBlock.length > 0) {
        ...
        if (textContent.trim()) {
          // marked.parse returns string | Promise<string>, but we expect string here
          // as we're not using async extensions
          const parsedHtml = marked.parse(textContent) as string;

          elements.push(
            <div
              key={`text-${currentTextStartLine}-${endLine}`}
              className="prose prose-sm max-w-none text-slate-300 mb-3"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(parsedHtml) }}
            />
          );
        }
        currentTextBlock = [];
