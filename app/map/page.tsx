...
import { getMapsPaginated, type PaginatedMapsResult } from "@/lib/supabase/maps";
...
export default function MapsPage() {
  ...
  // 🚀 FAST: Load initial maps (only 12)
  const loadInitialMaps = async () => {
    try {
      setLoading(true);
      const result = await getMapsPaginated(1, 12);
      
      setMaps(result.maps);
      setCurrentPage(1);
      setHasMore(result.hasMore);
      setTotalMaps(result.total);
    } catch (err) {
      console.error("Error loading initial maps:", err);
      toast({
        title: "Error",
  ...
  // 🚀 INFINITE SCROLL: Load more maps
  const loadMoreMaps = async () => {
    ...
    try {
      ...
      const result = await getMapsPaginated(nextPage, 12);
      
      setMaps(prev => [...prev, ...result.maps]);
  ...
  const getVinylColorsFromMetadata = (metadata: any) => {
    ...
    if (storedColors && storedColors.length >= 3) {
      ...
      const brightColor = color3; // Brightest color
      
      return {
        bg: `linear-gradient(135deg, rgba(${color1.r}, ${color1.g}, ${color1.b}, 0.9), rgba(${color2.r}, ${color2.g}, ${color2.b}, 0.8), rgba(${color3.r}, ${color3.g}, ${color3.b}, 0.9))`,
        grooveStyle: { borderColor: `rgba(${brightColor.r}, ${brightColor.g}, ${brightColor.b}, 0.6)` },
        labelStyle: { 
  ...
  const MapCard = ({ map, isAboveFold = false }: { map: MapWithStats; isAboveFold?: boolean }) => {
          ...
          <div className="absolute left-1/2 -translate-x-1/2 -top-16 w-72 h-72 transition-all duration-700 group-hover:-translate-y-20" style={{ zIndex: 1 }}>
            <div className="relative w-full h-full">
              ...
              <div className={`vinyl-record w-full h-full rounded-full shadow-2xl border-4 relative overflow-hidden ${
                ...
                {/* Vinyl Grooves */}
                <div className="absolute inset-4 rounded-full border opacity-40" style={vinylColors.grooveStyle} />
                <div className="absolute inset-8 rounded-full border opacity-25" style={vinylColors.grooveStyle} />
                <div className="absolute inset-12 rounded-full border opacity-15" style={vinylColors.grooveStyle} />
                <div className="absolute inset-16 rounded-full border opacity-10" style={vinylColors.grooveStyle} />
                
                {/* Center Label */}
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full border-2 flex items-center justify-center shadow-lg" style={vinylColors.labelStyle}>
      ...
      <div key={section.type} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-y-20 justify-items-center" style={{marginTop: '5rem'}}>
          {section.maps.map((map, index) => (
            <MapCard key={map.id} map={map} isAboveFold={index < 3} />
          ))}
        </div>
      </div>
    ));
        ...
        {!loading && maps.length > 0 && (
          <div className="text-center py-4">
            <p className="text-gray-400 text-sm">
              Showing {maps.length} of {totalMaps} maps
