# PassionSeed - FI Program TODO

**Last Updated**: March 18, 2026 (Sprint 8 completed)
**Current Sprint**: 10 (Product Development)
**Location**: San Francisco (3 months) → Bangkok

---

## 🟡 Current Sprint (In Progress)

### Sprint 10: Product Development
**Due**: Mar 25, 2026

| Step | Task | Hours | Status | Evidence Needed |
|------|------|-------|--------|-----------------|
| 1 | Growth | 4 | ❌ Not done | Weekly growth goals progress |
| 2 | Intellectual Property | 2 | ❌ Not done | IP assessment |
| 3 | Patents | 2 | ❌ Not done | Patent research |
| 4 | Solution Goals | 1 | ❌ Not done | Solution goals document |
| 5 | Project Team | 3 | ❌ Not done | Team structure |
| 6 | Product Roadmap | 2 | ❌ Not done | Roadmap document/link |
| 7 | Project Plan | 2 | ❌ Not done | Project plan |
| 8 | Development Tools | 3 | ❌ Not done | Tools chosen + why |
| 9 | Next Release Plan | 2 | ❌ Not done | Release plan |
| 10 | Advisor Follow-up | 4 | ❌ Not done | Advisor meeting notes |
| 11 | Second Mailing | 1 | ❌ Not done | Email sent to list |
| 12 | Mentor Office Hours | 0.5 | ❌ Not done | Scheduled |
| 13 | Investor Progress Review | 4 | ❌ Not done | Pitch deck + video |

**Note**: Investor Progress Review is the **second critical milestone** - present to investors/advisors for ratings.

---

## 🟢 Upcoming Sprints

### Sprint 11: Investor Progress Review
**Status**: Upcoming
**Key Tasks**:
- Present to investor panel
- Receive ratings (must score 2.0+ to continue)
- Epic Sprint if needed

### Sprint 12: Co-Founders & Team
**Status**: Upcoming
**Key Tasks**:
- Formalize co-founder agreements
- Equity documentation
- Team hiring plan

### Sprint 13: Growth
**Status**: Upcoming
**Key Tasks**:
- Growth metrics tracking
- Retention optimization
- Scale operations

### Sprint 14: Funding
**Status**: Upcoming
**Key Tasks**:
- Fundraising execution
- Investor meetings
- Term sheet negotiation

### Sprint 15: Graduation
**Status**: Upcoming
**Key Tasks**:
- Final pitch
- Graduation ceremony
- Alumni network onboarding

---

## 📋 Evidence Checklist

### Items with NO Evidence Yet

| Item | Sprint | Action Required |
|------|--------|-----------------|
| Product Roadmap | 10 | Create visual roadmap |
| Project Plan | 10 | Document milestones & timeline |
| Development Tools | 10 | List tools + justification |
| Investor Progress Review Pitch | 10 | Prepare 3-min pitch + deck |

---

## 👥 Advisors

### Confirmed Advisors
| Name | Expertise | LinkedIn | Status |
|------|-----------|----------|--------|
| Kirk | TBD | https://www.linkedin.com/in/kirkjaa/ | ✅ Confirmed |

### Target Advisors (Outreach Pending)
| Name | Category | Company | Connector |
|------|----------|---------|-----------|
| Pawoot Pumtang | Startup | Tarad.com | Natcha |
| Amornthep Thaweesit | Startup | Wongnai | Chula network |
| Thanawat Malabuppha | Startup | Priceza | FI Thailand |
| Kavin Thongplod | Industry | EdTech Fund | Startup ecosystem |
| Piyachart Rattanapian | Industry | Chula Faculty of Ed | Chula network |
| Tanawat Hiranyavas | Industry | Ministry of Ed | Government |
| Teerawat Wutti | Technology | Google | Thai tech community |
| Thanachart R. | Technology | Thai startup | Tech network |
| Pitchaya T. | Technology | Agoda | Tech network |

---

## 🎯 This Week's Priorities

### Must Do (Mar 18-25)
1. **Complete Sprint 10** (Product Development)
   - Document product roadmap
   - Create project plan
   - Prepare for Investor Progress Review

2. **Build AI Quest Generator MVP**
   - Core feature for moat
   - Test with 10 students

3. **Advisor Follow-up**
   - Schedule meeting with Kirk (new advisor)
   - LinkedIn: https://www.linkedin.com/in/kirkjaa/

---

## 📊 Progress Summary

| Sprint | Status | Completion |
|--------|--------|------------|
| 1. Onboarding | ✅ Complete | 100% |
| 2. Accelerator Kickoff | ✅ Complete | 100% |
| 3. Vision & Mission | ✅ Complete | 100% |
| 4. Customer Development | ✅ Complete | 100% |
| 5. Revenue & Business Models | ✅ Complete | 100% |
| 6. Pitch Mastery | ✅ Complete | 100% |
| 7. Mentor Idea Review | ✅ Complete | 100% |
| 8. Legal & Equity | ✅ Complete | 100% |
| 9. Go-to-Market & Scale | ⚠️ Current | 0% |
| 10. Product Development | 🔵 In Progress | 0% |
| 11-15. Remaining | ⏳ Upcoming | 0% |

**Overall Progress**: 8/15 sprints complete (53%)

---

## 🔗 Quick Links

- **Executive Summary**: https://passionseed.org/fi/exec
- **Pitch Deck**: https://www.figma.com/deck/kpDcFFNH2q3RioFuQJdEBm
- **Landing Page**: https://passionseed.org
- **Epic Sprint Report**: https://www.passionseed.org/epic-sprint
- **FI Dashboard**: https://fi.co/enrolled/home

---

*Last Updated: March 18, 2026*